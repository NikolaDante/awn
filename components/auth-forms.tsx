...
export function AuthForm({ mode, recoveryAuthorized = false }: { mode: Mode; recoveryAuthorized?: boolean }) {
  return (
    <Suspense fallback={<main className="auth-page"><section className="auth-card" aria-busy="true"><p className="auth-intro">Loading…</p></section></main>}>
      <AuthFormContent mode={mode} recoveryAuthorized={recoveryAuthorized} />
    </Suspense>
  );
}
...
function AuthFormContent({ mode, recoveryAuthorized }: { mode: Mode; recoveryAuthorized: boolean }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    createClient().auth.getSession().then(({ data, error: sessionError }) => { if (active) setRecoveryState(!sessionError && recoveryFormAllowed(recoveryAuthorized, Boolean(data.session)) ? "valid" : "invalid"); });
    return () => { active = false; };
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (mode === "sign-in") {
      ...
      const { data, error: authError } = await supabase.auth.signUp({ email, password, options: { emailRedirectTo: callbackUrl(`/auth/callback?next=${encodeURIComponent(next)}`) } });
      if (authError) fail("We couldn’t create your account just now. Please try again."); else if (signupNeedsConfirmation(data.session)) { setVerificationPending(true); setStatus("Check your email to verify your account before signing in."); } else { router.replace(next); router.refresh(); }
...
export function SignOutButton({ variant = "sidebar", onSignedOut, redirectTo = "/" }: { variant?: "sidebar" | "mobile"; onSignedOut?: () => void; redirectTo?: string }) {
  const router = useRouter(); const [busy, setBusy] = useState(false);
  return <button type="button" className={variant === "mobile" ? "mobile-sign-out-button" : "sidebar-placeholder"} disabled={busy} aria-label="Sign out" onClick={async () => { setBusy(true); await createClient().auth.signOut(); onSignedOut?.(); router.replace(redirectTo); router.refresh(); }}>{variant === "mobile" && <AppIcon name="arrow" />}<span>{busy ? "Signing out…" : "Sign out"}</span></button>;
}

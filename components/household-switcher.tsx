...
export function HouseholdSwitcher({ mobile = false, afterSelect }: { mobile?: boolean; afterSelect?: () => void }) {
  const { activeHouseholdId, householdName, memberCount, isPersonal, households, switching, switchHousehold } = useFinancialProfile();
  ...
  const kind = !isPersonal || memberCount > 1 ? "Shared" : "Personal";
  ...
  return <div ref={root} className={`household-switcher${mobile ? " is-mobile" : ""}`}>
    ...
    {expanded && <div className="household-switcher-menu" role="menu" aria-label="Your Households">
      {households.map((household) => <button key={household.id} type="button" role="menuitemradio" aria-checked={household.id === activeHouseholdId} disabled={switching} onClick={() => select(household.id)}><span aria-hidden="true">{household.id === activeHouseholdId ? "✓" : ""}</span><span><strong>{household.name}</strong><small>{!household.isPersonal || household.memberCount > 1 ? "Shared" : "Personal"}</small></span></button>)}

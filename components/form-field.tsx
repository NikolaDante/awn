...
export function FormField({ label, optional, hint, error, className = "", children }: { label: string; optional?: boolean; hint?: string; error?: string; className?: string; children: ReactNode }) {
  ...
  return <label className={`form-field stable-form-field ${className}`.trim()}><span className="field-label"><span className="field-label-text">{label}</span>{optional && <small>Optional</small>}</span>{control}<small className={`field-message-slot${error ? " field-error" : ""}`} id={messageId}>{error || hint || "\u00a0"}</small></label>;
}

...
import { budgetTemplateAmounts, equalCategorySuggestions, suggestedBudgetBucket, BUDGET_TEMPLATES, type BudgetBucket, type BudgetGuideCategory, type BudgetTemplateKey } from "@/lib/budget-guide";
...
export function BudgetGuide({ currency, goals, household = false, back, cancel, accept }: {
  ...
  const nextFromTemplate = () => {
    ...
    setCategories([
      ...equalCategorySuggestions(buckets.essentials, "essentials"),
      ...equalCategorySuggestions(buckets.lifestyle, "lifestyle"),
    ]);
    setError(""); setStep(1);
  };
  const finish = () => {
  ...
  return <div className="budget-guide">
    ...
    {step === 1 && buckets && <section className="budget-guide-step"><p className="app-eyebrow">Step 2 of 4</p><h3>Shape category suggestions</h3><p className="form-help">These are suggestions, not rules. Unallocated spending room is valid.</p><div className="budget-guide-category-list">{categories.map((item, index) => <div className="budget-guide-category" key={`${index}-${item.category}`}><label className="form-field">Category<input list="awn-guide-categories" value={item.category} onChange={(event) => updateCategory(index, { category: event.target.value, bucket: suggestedBudgetBucket(event.target.value) })} /></label><label className="form-field">Bucket<select value={item.bucket} onChange={(event) => updateCategory(index, { bucket: event.target.value as BudgetBucket })}><option value="essentials">Essentials</option><option value="lifestyle">Lifestyle</option></select></label><label className="form-field">Suggestion<MoneyInput value={item.amount} onValueChange={(value) => updateCategory(index, { amount: value })} /></label><button className="text-button is-danger" type="button" onClick={() => setCategories((items) => items.filter((_, position) => position !== index))}>Remove</button></div>)}</div><datalist id="awn-guide-categories">{DEFAULT_CATEGORY_NAMES.map((name) => <option value={name} key={name} />)}</datalist><button className="text-button" type="button" onClick={addCategory}>Add category suggestion</button><div className="budget-guide-totals"><span>Spending budget <strong>{formatMoney(buckets.spending, currency)}</strong></span><span>Category suggestions <strong>{formatMoney(categoryTotal, currency)}</strong></span><span>Unallocated <strong>{formatMoney(buckets.spending - categoryTotal, currency)}</strong></span></div></section>}

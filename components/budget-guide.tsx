...
import { budgetTemplateAmounts, customBudgetAllocation, suggestedBudgetBucket, BUDGET_TEMPLATES, type BudgetBucket, type BudgetCustomAmounts, type BudgetGuideCategory, type BudgetTemplateKey } from "@/lib/budget-guide";
...
export type BudgetGuideResult = { overall: number; savingsGuidance: number; categories: BudgetGuideCategory[]; goals: BudgetGuideGoal[] };

export function BudgetGuide({ currency, goals, household = false, initialAmount = 0, back, cancel, accept }: {
  currency: Currency;
  ...
  const [custom, setCustom] = useState<BudgetCustomAmounts>({ essentials: 0, lifestyle: 0, savings: 0 });
  ...
  const customStatus = customBudgetAllocation(amount, custom);

  const nextFromTemplate = () => {
    if (amount <= 0) return setError("Enter a planning amount above zero.");
    if (!buckets) return setError(customStatus.remaining > 0 ? `Allocate the remaining ${formatMoney(customStatus.remaining, currency)}.` : `You’ve allocated ${formatMoney(Math.abs(customStatus.remaining), currency)} more than your planning amount.`);
    setCategories([]);
    setError(""); setStep(1);
  };
  const finish = () => {
    ...
    accept({ overall: buckets.spending, savingsGuidance: buckets.savings, categories: categories.filter((item) => item.amount > 0 && item.category.trim()), goals: decideLater ? [] : goalDrafts.filter((goal) => goal.amount > 0) });
  };
  ...
  return <div className="budget-guide">
    ...
    {step === 0 && <section className="budget-guide-step"><p className="app-eyebrow">Step 1 of 4</p><h3>Choose a planning starting point</h3><p className="form-help">Use an amount you are comfortable planning with. {household ? "AWN never asks for your partner’s income." : "You can revise every suggestion before saving."}</p><label className="form-field">{household ? "Amount to plan together each month" : "Amount to plan each month"}<MoneyInput value={amount} onValueChange={(value) => { setAmount(value); setError(""); }} /></label><div className="budget-template-grid">{Object.entries(BUDGET_TEMPLATES).map(([key, value]) => <button type="button" className={template === key ? "is-selected" : ""} onClick={() => { setTemplate(key as BudgetTemplateKey); setError(""); }} key={key}><strong>{value.label}</strong><span>{value.essentials}% essentials · {value.lifestyle}% lifestyle · {value.savings}% savings</span></button>)}<button type="button" className={template === "custom" ? "is-selected" : ""} onClick={() => { setTemplate("custom"); setError(""); }}><strong>Custom</strong><span>Choose money amounts that equal your planning total.</span></button></div>{template === "custom" && <><div className="budget-custom-amounts">{(["essentials", "lifestyle", "savings"] as const).map((key) => <label className="form-field" key={key}>{key === "savings" && household ? "Shared savings" : key[0].toUpperCase() + key.slice(1)}<MoneyInput value={custom[key]} onValueChange={(value) => { setCustom((current) => ({ ...current, [key]: value })); setError(""); }} /></label>)}</div><div className="budget-guide-totals"><span>Allocated <strong>{formatMoney(customStatus.allocated, currency)}</strong></span><span>Planning amount <strong>{formatMoney(amount, currency)}</strong></span></div>{customStatus.remaining !== 0 && <p className={`form-message ${customStatus.remaining > 0 ? "is-warning" : "is-error"}`} role="status">{customStatus.remaining > 0 ? `Allocate the remaining ${formatMoney(customStatus.remaining, currency)}.` : `You’ve allocated ${formatMoney(Math.abs(customStatus.remaining), currency)} more than your planning amount.`}</p>}</>}{buckets && <div className="budget-guide-buckets"><span>Essentials<strong>{formatMoney(buckets.essentials, currency)}</strong></span><span>Lifestyle<strong>{formatMoney(buckets.lifestyle, currency)}</strong></span><span>{household ? "Shared savings" : "Savings"}<strong>{formatMoney(buckets.savings, currency)}</strong></span><span>Spending budget<strong>{formatMoney(buckets.spending, currency)}</strong></span></div>}</section>}

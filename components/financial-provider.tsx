...
import { cloudFinancialIssue, loadCloudFinancialProfile, saveCloudFinancialProfile } from "@/lib/cloud-financial-repository";
...
export function FinancialProvider({ children, ownerId }: Readonly<{ children: React.ReactNode; ownerId: string }>) {
  ...
  return <FinancialContext.Provider value={{ profile, activeHouseholdId, ready, saving, issue, save, retry }}>{children}</FinancialContext.Provider>;
}

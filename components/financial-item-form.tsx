...
export function FinancialItemForm({ kind, existing, profile, onCancel, onSave }: { kind: FinancialItemKind; existing?: FinancialItem; profile: FinancialProfile; onCancel: () => void; onSave: (item: FinancialItem) => boolean | void }) {
  ...
  return <div className="cards-form onboarding-item-form">
    ...
    {kind === "account" && <div className="cards-form-row">
      <FormField label="Account type"><select value={accountType} onChange={(event) => setAccountType(event.target.value as Account["type"])}><option value="current">Current / checking</option><option value="savings">Savings</option><option value="cash">Cash</option></select></FormField>

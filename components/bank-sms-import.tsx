...
export function BankSmsImportDialog({ close }: { close: () => void }) {
  ...
  return <ModalDialog title={stage === "input" ? "Import bank SMS" : stage === "review" ? "Review import" : "Import complete"} eyebrow={stage === "result" ? "Bank SMS" : "Manual paste"} close={close} className="bank-sms-dialog" closeLabel="Close bank SMS import" closeOnBackdrop={!saving}>
    {stage === "input" && <div className="sms-import-stage"><div className="sms-import-stage-body"><div className="sms-import-intro"><p>Paste one or more bank transaction messages. AWN will review them before importing.</p><small>Manual paste only. AWN cannot access your SMS inbox.</small></div><p className="form-help">Currently supported: FAB</p><label className="form-field">Paste bank SMS messages<textarea className="sms-import-textarea" value={input} onChange={(event) => setInput(event.target.value)} placeholder="Paste one or more bank transaction messages" data-modal-initial-focus /></label>{error && <p className="form-message is-error" role="alert">{error}</p>}</div><div className="confirm-dialog-actions sms-import-footer"><button className="app-button app-button-secondary" type="button" onClick={close}>Cancel</button><button className="app-button" type="button" onClick={review} disabled={!historyReady}>Review messages</button></div></div>}
    {stage === "review" && <div className="sms-import-stage"><div className="sms-import-stage-body"><div className="sms-import-summary"><strong>{items.length} {items.length === 1 ? "message" : "messages"} detected</strong><span>Ready<b>{summary.ready}</b></span><span>Needs review<b>{summary.review}</b></span><span>Duplicates<b>{summary.duplicate}</b></span><span>Unsupported<b>{summary.unsupported}</b></span></div><p className="form-help">Imported transactions affect AWN balances just like manually added transactions. A bank’s observed balance is retained for reconciliation only and never overwrites AWN.</p><div className="sms-proposal-list">{items.map((item) => <ProposalCard key={item.proposal.id} item={item} profile={profile} expanded={expanded === item.proposal.id} toggle={() => setExpanded(expanded === item.proposal.id ? null : item.proposal.id)} update={(patch) => update(item.proposal.id, patch)} />)}</div>{unresolved > 0 && <p className="form-message is-error" role="alert">Resolve {unresolved} {unresolved === 1 ? "transaction" : "transactions"} before importing, or exclude them.</p>}{error && <p className="form-message is-error" role="alert">{error}</p>}</div><div className="confirm-dialog-actions sms-import-footer"><button className="app-button app-button-secondary" type="button" onClick={() => { setStage("input"); setError(""); }}>Back</button><button className="app-button" type="button" disabled={saving || !included.length || unresolved > 0} onClick={performImport}>{saving ? "Importing…" : `Import ${included.length} ${included.length === 1 ? "transaction" : "transactions"}`}</button></div></div>}

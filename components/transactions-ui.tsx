...
export function TransactionsView() {
  ...
  return <>{form && <TransactionForm editing={editing} close={() => { setForm(false); setEditing(undefined); }} />}{allOpen && <AllTransactionsDialog close={() => setAllOpen(false)} transactions={currentMonthTransactions} profile={profile} readOnly={false} edit={(item) => { setAllOpen(false); setEditing(item); setForm(true); }} remove={setDeleting} />}{deleting && <TransactionDeleteDialog transaction={deleting} close={() => setDeleting(undefined)} />}{categoriesOpen && <AllCategoriesDialog close={() => setCategoriesOpen(false)} categories={categories} profile={profile} />}
    <div className="transactions-overview"><section className="transactions-hero" aria-label={`${period.label} financial position`}><div className="transactions-hero-main"><div className="transactions-hero-heading"><div><p className="app-eyebrow">Current financial position</p><span>{period.label}</span></div><span className={`status-pill is-${budget.tone}`}>{budget.statusLabel}</span></div><AnimatedMoney className="transactions-hero-value" value={actual.currentPosition} currency={profile.currency} /><div className="transactions-hero-details"><span>Opening position<strong>{formatMoney(actual.openingPosition, profile.currency)}</strong></span><span>Budget<strong>{budget.budget === null ? "No budget" : formatMoney(budget.budget, profile.currency)}</strong></span><span>Budget difference<strong className={budget.kind === "over" ? "negative" : budget.kind === "none" ? "neutral" : "positive"}>{budget.remaining === null ? "Not available" : formatMoney(Math.abs(budget.remaining), profile.currency)}</strong></span></div></div></section><section className="transactions-period-kpis" aria-label="Budget-period totals"><TransactionHeroSummary kind="income" label="Period income" helper="Cash coming in" value={formatMoney(actual.income, profile.currency)} /><TransactionHeroSummary kind="expense" label="Period expenses" helper="Cash going out" value={formatMoney(actual.expenses, profile.currency)} /></section></div>

...
import { mutateLedger, normalizeTransaction, UNBUDGETED_CATEGORY } from "@/lib/financial-ledger";
...
export function TransactionForm({ editing, close }: { editing?: Transaction; close: () => void }) {
  ...
  const submit = () => {
    ...
    if (type === "transfer" && (!source || !target || source === target)) return setError("Choose two different places for the transfer.");
  ...
  return createPortal(<div className="dialog-backdrop app-dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section ref={dialogRef} tabIndex={-1} className="confirm-dialog transaction-form" role="dialog" aria-modal="true" aria-labelledby="transaction-title">
    ...
    <div className="transaction-form-body"><div className="transaction-fields"><label className="form-field">Amount<MoneyInput value={amount} onValueChange={setAmount} placeholder="0.00" /></label><label className="form-field">Date<input type="date" max={new Date().toLocaleDateString("en-CA")} value={date} onChange={(event) => setDate(event.target.value)} /></label>
      ...
      {type === "expense" && <><label className="form-field">Category (optional)<select value={category} onChange={(event) => setCategory(event.target.value)}><option value="">Choose a category</option>{category && !activeCategories.some((item) => item.name === category) && <option value={category}>{category}</option>}{activeCategories.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}</select></label><label className="form-field">Paid from<select value={paidFrom} onChange={(event) => setPaidFrom(event.target.value)}><option value="">Choose source</option>{groupedOptions(true, true)}</select></label>{(profile.debitCards ?? []).some((item) => !item.linkedAccountId) && <p className="form-help transaction-field-wide">Unlinked debit cards are unavailable until they are linked to an account in Cards &amp; Accounts.</p>}</>}
...
export function TransactionsView() {
  ...
  const netStatus = difference < 0 ? "Over budget" : difference <= spendingBudget * .15 ? "Near limit" : "Under budget";
  const remove = (item: Transaction) => { if (!window.confirm("Delete this transaction? Its full financial effect will be reversed.")) return; const result = mutateLedger(profile, { kind: "delete", id: item.id }); if (result.ok) save(result.profile); else window.alert(result.error); };

...
test("the importer uses the shared modal, cloud import RPC, and enabled Dashboard and Transactions entry points", () => {
  ...
  assert.match(ui, /<ModalDialog/); assert.match(ui, /Import bank SMS/); assert.match(ui, /Manual paste only/); assert.match(ui, /Currently supported: FAB/); assert.match(ui, /AWN will review them before importing/); assert.match(ui, /Paste one or more bank transaction messages/); assert.match(ui, /Resolve \{unresolved\}/); assert.match(ui, /expanded \? "Done" : "Edit"/); assert.doesNotMatch(ui, /FAB_LABEL|<label className="form-field">Bank|parseBankSms\("fab"|WebOTP|navigator\.sms|provider_token/);

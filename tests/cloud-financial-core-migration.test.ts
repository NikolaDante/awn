...
test("orders one additive financial-core migration after two immutable applied migrations", () => {
  const migrationNames = readdirSync(migrationsDirectory).filter((name) => name.endsWith(".sql")).sort();
  assert.deepEqual(migrationNames, [initialName, repairName, coreName]);
